...
/**
 * retry-x CLI - Testing and demonstration tool
 * 
 * A command-line interface for testing the retry-x library with different scenarios
 * and configurations.
 */

import { retry, RetryOptions } from '../index.js';
...
// Test command - simulate different failure scenarios
program
  .command('test')
  ...
  .action(async (options) => {
    ...
    const retryOptions: RetryOptions = {
      ...
      jitter: !!jitter,
      timeout: timeout ? parseInt(timeout) : undefined as any,
    ...
    try {
      ...
      console.log(`📊 Stats:`, (error as any).stats);
    }
  });

// Benchmark command - test performance
program
  .command('benchmark')
  ...
  .action(async (options) => {
    ...
    for (let i = 0; i < parseInt(iterations); i++) {
      try {
        ...
        results.push((error as any).stats);
      }
    }
...
// Example command - show usage examples
program
  .command('example')
...
// Info command - show library information
program
  .command('info')
